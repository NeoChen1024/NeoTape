...
- **Channel** partitions a slice into `ch_metadata` and `ch_content`. Metadata, when present, precedes content. A slice may contain only metadata.
...
Within a slice tape file, frames are located by `frame_payload_size`, not by additional filemarks. Metadata frames, when present, precede content frames. A slice MUST contain at least one frame across its channels.
...
A slice consists of one or more frames across one or two channels:

```
Slice[k] =
    (optional) ch_metadata.Frame[0].payload + ... + ch_metadata.Frame[M-1].payload +
    (optional) ch_content.Frame[0].payload + ... + ch_content.Frame[N-1].payload
```
...
- `channel_frame_seq_num = 0` — first frame of the current channel group.
