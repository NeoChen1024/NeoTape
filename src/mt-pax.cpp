...
CliOptions parse_args(int argc, char **argv) {
	...
	while ((c = getopt_long(argc, argv, "C:f:P:vxh", long_opts, nullptr)) != -1) {
		switch (c) {
		case 'C': opts.writer.chdir_dir = optarg; break;
		case 'f': opts.output = optarg; opts.writer.output_name = optarg; break;
		case 'P': {
			char *end = nullptr;
		...
		case 'v': opts.writer.verbose = std::min(opts.writer.verbose + 1, 2); break;
		case 'x': opts.writer.one_file_system = true; break;
		case 256:
			try {
		...
		case 258:
			...
			break;
		case 259: opts.slice_output_prefix = optarg; break;
...
neotape::PaxWriterCallbacks continuous_callbacks(const string &output,
    FILE *&out_file, bool &close_file) {
	if (output == "-") {
		out_file = stdout;
		close_file = false;
	} else {
		out_file = std::fopen(output.c_str(), "wb");
		if (!out_file) fail_errno(string("open ") + output);
		close_file = true;
	}
	...
	return neotape::PaxWriterCallbacks{
	    ...
	    .write_chunk = [&](neotape::PaxChunk chunk) {
		    if (std::fwrite(chunk.bytes.data(), 1, chunk.bytes.size(), out_file) !=
		        chunk.bytes.size())
...
int main(int argc, char **argv) {
	try {
		...
		if (opts.slice_output_prefix.has_value())
			opts.slice_output_prefix = fs::absolute(*opts.slice_output_prefix).string();
		neotape::PaxWriterCallbacks callbacks = opts.slice_output_prefix.has_value()
		    ? slice_callbacks(*opts.slice_output_prefix, out_file)
		    : continuous_callbacks(opts.output, out_file, close_file);

		neotape::PaxWriteResult result = neotape::write_pax(opts.writer,
		    std::move(callbacks));
		if (close_file && out_file != nullptr && std::fclose(out_file) != 0)
			fail_errno(string("close ") + opts.output);

		std::cerr << format("\n{}  {}\n", result.blake3_hex,
		    opts.slice_output_prefix.has_value() ? *opts.slice_output_prefix : opts.output);
		return 0;
	} catch (const std::exception &e) {
		fail(e.what());
	}
