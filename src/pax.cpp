...
void usage(const char *prog) {
    std::cerr << format("usage: {} -f <out-file|-> [-v|-vv] [-x] [-C <dir>] "
                        "<path> [path ...]\n",
                        prog);
}
...
int output_open_callback(archive *, void *) { return ARCHIVE_OK; }

la_ssize_t output_write_callback(archive *a, void *client_data,
...
Options parse_args(int argc, char **argv) {
    ...
    while ((c = getopt_long(argc, argv, "C:f:vxh", long_opts, nullptr)) != -1) {
        ...
        case 'f':
            ...
            break;
        case 'v':
            opts.verbose = std::min(opts.verbose + 1, 2);
            break;
        case 'x':
...
void mark_link_target_as_utf8(archive_entry *entry) {
    ...
    else if (const char *hardlink = archive_entry_hardlink(entry);
             hardlink != nullptr)
...
string entry_timestamp(archive_entry *entry) {
    ...
    char buffer[32]{};
    if (std::strftime(buffer, sizeof(buffer), "%Y%m%dT%H%M%S%z", &local_time) ==
        0)
        return "00000000T000000+0000";
    return buffer;
}
...
string entry_display_path(archive_entry *entry) {
    string path = archive_entry_pathname(entry) != nullptr
                      ...
                      : "";
    if (archive_entry_filetype(entry) == AE_IFDIR && !path.empty() &&
        path.back() != '/')
        path += '/';
    return path;
}
...
void copy_file_data(archive *writer, archive_entry *entry, int fd) {
    ...
    for (;;) {
        ...
        if (n == 0)
            break;
        ssize_t written =
            archive_write_data(writer, buffer.data(), static_cast<size_t>(n));
        if (written < 0) {
            close(fd);
...
bool write_archive_entry(archive *writer, const Options &opts,
                         archive_entry *entry) {
    ...
    check_archive(archive_write_finish_entry(writer), writer,
                  "finish archive entry");
    return true;
}

void write_linkified_entry(archive *writer, const Options &opts,
                           archive_entry *entry) {
    if (entry == nullptr)
        return;
...
void write_source(archive *writer, const Options &opts,
                  const neotape::SourceSpec &source,
                  archive_entry_linkresolver *resolver) {
    ...
    for (;;) {
        ...
        if (source_path != nullptr) {
            ...
            string archive_path =
                neotape::archive_path_for_source(source, source_path);
...
void write_pax_archive(const Options &opts) {
    ...
    check_archive(archive_write_add_filter_none(writer), writer,
                  "set uncompressed output");
    check_archive(archive_write_set_format_pax(writer), writer,
                  "set pax format");
    check_archive(
        archive_write_set_options(writer, "xattrheader=ALL,hdrcharset=UTF-8"),
    ...
    check_archive(archive_write_open(writer, &output, output_open_callback,
                                     output_write_callback,
                                     output_close_callback),
                  writer, "open output");
