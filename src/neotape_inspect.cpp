...
void inspect_volume(const fs::path &path, InspectState &state,
    ...
    std::cout << format("{}: volume seq={} block={} archive={}\n",
                        path.string(), header.volume_seq_num,
                        header.volume_block_size, header.archive_uuid);
...
void inspect_frame(const fs::path &path, InspectState &state,
    ...
    require(bytes.size() == header.volume_block_size,
            format("{}: frame record size does not match block size",
                   path.string()));
    require(header.frame_payload_size <=
        ...
        if (start) {
            require(!state.slice_open,
                    format("{}: new slice starts before previous slice ended",
                           path.string()));
            require(header.logical_slice_seq_num ==
    ...
    std::cout << format(
        "{}: frame global={} slice={} within={} payload={} type={} "
        "flags=0x{:04x}\n",
        path.string(), header.global_frame_seq_num,
    ...
    if (parsed.volume)
        ...
        for (size_t offset = 0; offset < bytes.size();
            ...
            require(frame.frame.has_value(),
                    format("{}: non-frame record inside slice file",
                           path.string()));
