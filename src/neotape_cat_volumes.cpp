...
public:
	...
	void write(const uint8_t *data, size_t len) override {
		if (std::fwrite(data, 1, len, file_) != len)
			throw std::runtime_error(
			    format("write: {}", std::strerror(errno)));
	}
	...
	while (vol.next_file()) {
		while (vol.read_record(record)) {
			...
			if (parsed.frame) {
				...
				if (f.frame_content_type == neotape::FrameContentType::slice_content) {
					bool start = (f.flags & neotape::frame_flag_start) != 0;
					bool end = (f.flags & neotape::frame_flag_end) != 0;

					if (start) {
						if (rs.slice_open)
							fail(format("new slice {} starts before previous slice ended",
							    f.logical_slice_seq_num));
						if (f.logical_slice_seq_num != rs.expected_logical_slice_seq_num)
					...
					if (!rs.slice_open)
						fail(format("content frame outside slice at global frame {}",
						    f.global_frame_seq_num));

					if (f.global_frame_seq_num != rs.expected_global_frame_seq_num)
						fail(format("expected global frame {}, got {}",
						    rs.expected_global_frame_seq_num,
						    f.global_frame_seq_num));

					auto payload_offset = neotape::fixed_header_size;
					auto payload_size = static_cast<size_t>(f.frame_payload_size);

					rs.sink->write(record.data() + payload_offset, payload_size);

					blake3_hasher_update(&rs.slice_hasher,
					    record.data() + payload_offset, payload_size);
					rs.current_slice_size += payload_size;

					if (end) {
						neotape::Hash slice_hash{};
						blake3_hasher_finalize(&rs.slice_hasher,
						    slice_hash.data(), slice_hash.size());

						if (f.slice_content_size != rs.current_slice_size)
							fail(format("slice {} size mismatch: declared {} actual {}",
							    f.logical_slice_seq_num,
							    f.slice_content_size,
			...
			if (parsed.archive_end) {
				...
				if (ae.last_global_frame_seq_num + 1 != rs.expected_global_frame_seq_num)
					fail(format("archive end frame seq mismatch: declared {} expected {}",
